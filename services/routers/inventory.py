"""Inventory management endpoints backed by Supabase PostgreSQL."""
...
from user_models import UserPublic
...
def list_products(session: Session = Depends(get_db)) -> list[SKUResponse]:
...
def get_product(product_id: int, session: Session = Depends(get_db)) -> SKUResponse:
...
def list_orders(session: Session = Depends(get_db)) -> list[OrderResponse]:
    inbound_rows = session.exec(
        select(StockEntry, SKU).join(SKU, StockEntry.sku_id == SKU.id)
    ).all()
    outbound_rows = session.exec(
        select(StockExit, SKU).join(SKU, StockExit.sku_id == SKU.id)
    ).all()
